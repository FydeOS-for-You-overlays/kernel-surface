...
extern void* zx_get_page(struct os_pages_memory *memory, int page_num);
extern unsigned int zx_get_page_size(struct os_pages_memory *memory);
extern void zx_set_page_dma_addr(struct os_pages_memory *memory, int page_num, struct zx_dma_addr_t *zx_dma_addr);
extern struct zx_dma_addr_t* zx_get_page_dma_addr(struct os_pages_memory *memory, int page_num);
extern int zx_dma_mapping_error(void *device, struct zx_dma_addr_t *dma_addr);
extern struct zx_dma_addr_t* zx_dma_map_page(void *device, void *page, unsigned int offset, unsigned int size, unsigned int dir);
extern void zx_dma_unmap_page(void *device,  struct zx_dma_addr_t *zx_dma_addr, unsigned int size, unsigned int dir);
extern unsigned long long zx_fetch_iov_addr(struct zx_dma_addr_t *zx_dma_addr);
