...
static void zx_gem_dmabuf_release(struct dma_buf *dma_buf)
{
...
int zx_drm_gem_create_resource_ioctl(struct drm_file *file, zx_create_resource_t *create)
{
    ...
    zxki_ALLOCATIONINFO stack_kinfos[8];
    zxki_ALLOCATIONINFO *kinfo = stack_kinfos;
    zxki_ALLOCATIONINFO __user *uinfo = (void*)(unsigned long)create->pAllocationInfo;
    struct drm_zx_gem_object *stack_objs[8];
    struct drm_zx_gem_object **objs = stack_objs;
    zx_query_info_t query = {0, };
    ...
    {
        objs = zx_malloc(obj_count * sizeof(*objs));
    }
...
#endif
{
    ...
    page_num = NUM_PAGES(obj->map_argu.size);
