...
static int zx_alloc_pmem_block(struct os_pages_memory *memory, int alloc_size, int block_index, int area_index)
{
    ...
    //zx_info("phy_addr: %x, block[%04d], size: %dK, area[%04d], area_size: %dk.\n", 
    //     page_to_phys(block->pages), block_index, alloc_size>>10, area_index, memory->area_size >> 10);
...
struct os_pages_memory *zx_allocate_pages_memory_priv(int size, int page_size, alloc_pages_flags_t alloc_flags)
{
    ...
    memory->dma_addr    = zx_calloc(memory->area_cnt * sizeof(struct zx_dma_addr_t *));
...
void* zx_get_page(struct os_pages_memory *memory, int page_num)
{
    struct os_pmem_area *area = &memory->areas[page_num];
    return area->block->pages;
}

unsigned int zx_get_page_size(struct os_pages_memory *memory)
{
    return memory->page_size;
}

void zx_set_page_dma_addr(struct os_pages_memory *memory, int page_num, struct zx_dma_addr_t *zx_dma_addr)
{
    memory->dma_addr[page_num] = zx_dma_addr;
}

struct zx_dma_addr_t *zx_get_page_dma_addr(struct os_pages_memory *memory, int page_num)
{
    return memory->dma_addr[page_num];
}

struct zx_dma_addr_t *zx_dma_map_page(void *device, void *page, unsigned int offset, unsigned int size, unsigned int dir)
{
    struct zx_dma_addr_t *zx_dma_addr  = zx_calloc(sizeof(struct zx_dma_addr_t));

    if(!zx_dma_addr)
        return NULL;

    zx_dma_addr->dma_addr = dma_map_page(device, page, offset, size, dir);

    return zx_dma_addr;
}

void zx_dma_unmap_page(void *device, struct zx_dma_addr_t* zx_dma_addr, unsigned int size, unsigned int dir)
{
    dma_unmap_page(device, zx_dma_addr->dma_addr, size, dir);

    zx_free(zx_dma_addr);
}

int zx_dma_mapping_error(void *device, struct zx_dma_addr_t *zx_dma_addr)
{
    return dma_mapping_error(device, zx_dma_addr->dma_addr);
}

unsigned long long zx_fetch_iov_addr(struct zx_dma_addr_t *zx_dma_addr)
{
    return (unsigned long long)zx_dma_addr->dma_addr;
}
