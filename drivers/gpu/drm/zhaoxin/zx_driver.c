...
static krnl_import_func_list_t zx_export =
{
    ...
    .dma_map_page                  = zx_dma_map_page,
    .dma_unmap_page                = zx_dma_unmap_page,
    .get_page                      = zx_get_page,
    .get_page_size                 = zx_get_page_size,
    .get_page_dma_addr             = zx_get_page_dma_addr,
    .set_page_dma_addr             = zx_set_page_dma_addr,
    .dma_mapping_error             = zx_dma_mapping_error,
    .fetch_iov_addr                = zx_fetch_iov_addr,
...
int zx_card_init(zx_card_t *zx, void *pdev)
{
    ...
    // 1, if we are primary, our drmfb is registered and fbcon will switch to it and call set_par to set vesa mode, 
    //     and cbios will enable linear address after setmode finished.
    // 2, if we are secondary, our post function shoule be called, and cbpost will also enable linear address mode.
    //     seems linear address bit can't be 0 at this point. Is some other case in which linear address is disabed exist?
    // I don't know the case right now. Just add this temp code here as we need write video memory to initialize engine now. 
...
#else
    proc_create_data(ZX_PROC_NAME, 0, NULL, &zx_proc_fops, zx);
#endif
